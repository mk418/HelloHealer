...
- [x] Caster-knowledge downranking — `Bindings:Resolve` degrades a stored rank the player hasn't trained yet (default level-60 ranks on a leveling character) to the highest known rank, so the click works while leveling instead of silently failing; the tooltip flags any substitution in yellow
- [x] Target-aware buff downranking — `Bindings:ResolveForTarget` picks the highest rank of a scaling group buff (Fortitude/Prayer of Fortitude, Divine Spirit/Prayer of Spirit, Mark of the Wild/Gift of the Wild, Thorns) that the *target's* level can receive, so buffing a low-level character lands a working rank instead of erroring "Target is too low level". Group/tank/player cells bake the rank from their unit's level (re-baked on `GROUP_ROSTER_UPDATE` / `UNIT_LEVEL`); the Target cell's `@target` branch re-bakes on `PLAYER_TARGET_CHANGED`. The gate is the rank's train level — always ≥ the engine's true target floor, so the result is guaranteed castable (at worst one rank shy for targets within a few levels of max). `BUFF_TARGET_LEVELS` holds the per-rank levels (verified against the Classic Era spell DB); buffs not in the table keep plain caster-knowledge resolution. Capped at the rank the user stored / can cast, so it never *up*ranks past an explicit choice
